# agent/v2/capabilities/cancellation.py
#
# CANCELLATION — the first V2 capability with a sensitive tool, proving the
# full confirmation/idempotency cycle (plan section 20-21, 25). Reuses
# agent/tools.py's get_cancellation_eligible_orders/submit_order_cancellation
# directly (imported, not forked) via v2/tools/cancellation_tools.py.

from v2.capabilities.types import Capability
from v2.tools.cancellation_tools import (
...
CANCELLATION_CAPABILITY = Capability(
    ...
    description_for_classifier=(
        "Customer wants to stop/cancel an order or specific items before "
        "it ships."
    ),
    ...
    mandatory_a2ui=["cancellationConfirmationPending", "cancellationConfirmed"],
